...
class Invoice(models.Model):
    ...
    invoice_customer = models.ForeignKey('Customer', on_delete=models.SET_NULL, null=True)
...
class InventoryLog(models.Model):
    ...
    def __str__(self):
        ...
        return f"{product_name} | {self.change} | {self.description} | {self.date}"
...
class Inventory(models.Model):
    ...
    def __str__(self):
        if self.product:
            return f"{self.product.product_name} | {self.current_stock} | {self.last_log}"
        else:
            return f"Inventory Entry | {self.current_stock} | {self.last_log}"
...
class BookLog(models.Model):
    ...
    def __str__(self):
        if self.parent_book and self.parent_book.customer:
            return (
                self.parent_book.customer.customer_name + " | " + 
                str(self.change) + " | " + 
                self.description + " | " + 
                str(self.date)
            )
        else:
            return "BookLog with missing customer information"
